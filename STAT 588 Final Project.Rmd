...
Checking above for some batch effect ... it does not look good. They used MAS5 to normalize. Bad data was already removed (85 subjects, but only 75 CEL files). 
...
Normalized data using the study's method above
...
Data is normalized and stored in a new object.

```{r}
#normdata.2<-affy::rma(rawdata)
```

Tried normalizing the data using the class method instead just to see what it looks like.
...
This shows things in log, but this isn't what researchers used so I was just curious.

```{r}
dat<-gset@phenoData@data
```

We will go with MAS 5 b/c even though RMA produces more reproducable results, the experiment was designed with MAS 5 so we'd get different stuff if we used RMA. RMA is also considered very harsh as it's a multiarray method and MAS 5 normalizes chips independently. 
...
test$p.value
```
 T-test show that there's no difference in type of smoker with top differentially expressed genes. All smoking is bad!
...
Sex doesn't impact the top differentially expressed gene as heavily as smoking status.
...
Race doesn't impact the top differentially expressed gene as heavily as smoking status.
...
No correlation between age and expression in the top differentially expressed genes!
